...
import { isMobileDevice, getMobileStyles, getDesktopStyles } from "~/utils/deviceDetection";
import { useEffect, useState } from "react";

// サーバーサイドでユーザーエージェントを検出してモバイルかどうかを判定
...
export default function Home() {
    ...
    // モバイルフラグをステートとして管理し、サーバーサイドの値で初期化
    ...
    // デバイスタイプに応じたスタイルを取得
    const deviceStyles = isMobileView ? getMobileStyles() : getDesktopStyles();
    ...
    return (
        <div className={`${styles.Home} ${mobileClass}`}>
            <Header />
            <div className={styles.item} style={deviceStyles.homeItem}>
                <div className={styles.icon} style={deviceStyles.homeIcon}></div>
                <div className={styles.myName}>
                    <h1>Koshi Tanaka</h1>
                    <small>@kokokoko0825</small>
                    <h2>Software Engineer</h2>
                </div>
                <div className={styles.accontFrame} style={deviceStyles.accountFrame}>
